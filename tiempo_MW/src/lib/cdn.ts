...
// Ciudades de Medio Oeste en weather.json (capitales + latinas del feed
...
const CONDITION_CITIES: { key: string; name: string; lon: number; lat: number }[] = [
  ...
  { key: "Detroit|Michigan", name: "Detroit", lon: -83.05, lat: 42.33 },
  { key: "Minneapolis|Minnesota", name: "Mineápolis", lon: -93.27, lat: 44.98 },
  { key: "St. Louis|Missouri", name: "San Luis", lon: -90.2, lat: 38.63 },
  { key: "Kansas City|Missouri", name: "Kansas City", lon: -94.58, lat: 39.1 },
  { key: "Indianapolis|Indiana", name: "Indianápolis", lon: -86.16, lat: 39.77 },
...
// Aeropuertos de Medio Oeste. Los que falten en el feed FAA de nimbus se
...
const AIRPORT_CATALOG: { iata: string; city: string; lon: number; lat: number }[] = [
  { iata: "ORD", city: "Chicago O'Hare", lon: -87.9, lat: 41.98 },
  { iata: "MDW", city: "Chicago Midway", lon: -87.75, lat: 41.79 },
  { iata: "DTW", city: "Detroit", lon: -83.35, lat: 42.21 },
  { iata: "MSP", city: "Mineápolis", lon: -93.22, lat: 44.88 },
  { iata: "STL", city: "San Luis", lon: -90.37, lat: 38.75 },
  { iata: "MCI", city: "Kansas City", lon: -94.71, lat: 39.3 },
  { iata: "IND", city: "Indianápolis", lon: -86.29, lat: 39.72 },
  { iata: "CMH", city: "Columbus", lon: -82.89, lat: 40.0 },
  { iata: "CLE", city: "Cleveland", lon: -81.85, lat: 41.41 },
...
// Medio Oeste. Los ids que el feed de nimbus aún no publique se descartan
...
const SERVICE_CITIES: { id: string; name: string; lon: number; lat: number }[] = [
  ...
  { id: "DET", name: "Detroit", lon: -83.05, lat: 42.33 },
  { id: "STL", name: "San Luis", lon: -90.2, lat: 38.63 },
  { id: "KC", name: "Kansas City", lon: -94.58, lat: 39.1 },
  { id: "IND", name: "Indianápolis", lon: -86.16, lat: 39.77 },
  { id: "CMH", name: "Columbus", lon: -82.99, lat: 39.96 },
  { id: "CLE", name: "Cleveland", lon: -81.69, lat: 41.5 },
  { id: "CVG", name: "Cincinnati", lon: -84.51, lat: 39.1 },
  { id: "MKE", name: "Milwaukee", lon: -87.91, lat: 43.04 },
  { id: "DSM", name: "Des Moines", lon: -93.63, lat: 41.59 },
  { id: "GRR", name: "Grand Rapids", lon: -85.67, lat: 42.96 },
...
export async function fetchSpcOutlook(signal?: AbortSignal): Promise<SpcOutlook | null> {
  try {
    ...
    return {
      ...
      hail: arr("hail"),
      prob: arr("prob"),
      // population_by_level_MW (no la nacional): el titular de población debe
      // reflejar el ENCUADRE Medio Oeste. Sin fallback a propósito.
      populationByLevel:
        d?.population_by_level_mw && typeof d.population_by_level_mw === "object"
          ? d.population_by_level_mw
          : undefined,
    };
  } catch (e) {
...
// Medio Oeste. Los ids que ya publica el feed nacional de tmax se usan tal
...
export async function fetchTmaxCities(
  ...
  try {
    ...
    if (r.ok) {
      ...
      // population_mw (no population): la tarjeta debe mostrar la población del
      // ENCUADRE Medio Oeste, no la nacional. Sin fallback a propósito.
      popToday = pop(d?.population_mw?.today);
      popTomorrow = pop(d?.population_mw?.tomorrow);
    }
  } catch (e) {
...
export async function fetchStormReports(signal?: AbortSignal): Promise<StormReportsData | null> {
  try {
    ...
    if (!r.ok) return null;
    const d = await r.json();
    // Bloque REGIONAL del feed (regions.mw): total/summary/puntos SOLO del
...
// del MEDIO OESTE un sismo dentro del encuadre (incluye New Madrid en el
...
export async function fetchQuake(signal?: AbortSignal): Promise<Quake | null> {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // dentro del encuadre Medio Oeste.
...
export const ESCALETA_SLUG = "tiempo-mw";
