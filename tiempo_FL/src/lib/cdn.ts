...
// bbox es un rectángulo) pero SIN ciudades ni datos de fuera de FL — esos van en
// tiempo_SE. (Mantiene el nombre CONUS_VIEW para no tocar las escenas.)
...
// Ciudades de FL presentes en weather.json. El feed nacional es por capitales +
// latinas, así que de serie solo trae Miami, Orlando y Tallahassee; el resto las
// inyecta cities_weather_pipeline.py en nimbus con observaciones NWS (mismo
// mecanismo que Nueva Orleans en tiempo_SE) — si el feed aún no trae alguna,
...
const CONDITION_CITIES: { key: string; name: string; lon: number; lat: number }[] = [
  ...
  { key: "Orlando|Florida", name: "Orlando", lon: -81.38, lat: 28.54 },
  { key: "Tallahassee|Florida", name: "Tallahassee", lon: -84.28, lat: 30.44 },
  { key: "Tampa|Florida", name: "Tampa", lon: -82.46, lat: 27.95 },
  { key: "Jacksonville|Florida", name: "Jacksonville", lon: -81.66, lat: 30.33 },
  { key: "Key West|Florida", name: "Cayo Hueso", lon: -81.78, lat: 24.56 },
  { key: "Fort Myers|Florida", name: "Fort Myers", lon: -81.87, lat: 26.64 },
  { key: "Pensacola|Florida", name: "Pensacola", lon: -87.22, lat: 30.42 },
];
...
// Aeropuertos de FL. Los que falten en el feed FAA de nimbus se añaden a
// DELAYS_IATA allí; hasta entonces se descartan (el catálogo decide qué se dibuja).
const AIRPORT_CATALOG: { iata: string; city: string; lon: number; lat: number }[] = [
  { iata: "MIA", city: "Miami", lon: -80.29, lat: 25.8 },
  { iata: "FLL", city: "Fort Lauderdale", lon: -80.15, lat: 26.07 },
  { iata: "PBI", city: "West Palm Beach", lon: -80.1, lat: 26.68 },
  { iata: "EYW", city: "Cayo Hueso", lon: -81.76, lat: 24.56 },
  { iata: "RSW", city: "Fort Myers", lon: -81.76, lat: 26.54 },
  { iata: "TPA", city: "Tampa", lon: -82.53, lat: 27.98 },
  { iata: "SRQ", city: "Sarasota", lon: -82.55, lat: 27.4 },
  { iata: "MCO", city: "Orlando", lon: -81.31, lat: 28.43 },
  { iata: "DAB", city: "Daytona Beach", lon: -81.06, lat: 29.18 },
  { iata: "JAX", city: "Jacksonville", lon: -81.69, lat: 30.49 },
  { iata: "TLH", city: "Tallahassee", lon: -84.35, lat: 30.4 },
  { iata: "PNS", city: "Pensacola", lon: -87.19, lat: 30.47 },
];
...
// FL. Los ids que el feed de nimbus aún no publique se descartan hasta que
// uv_pipeline/aqi_pipeline los incluyan (mismo patrón que NE/SE).
const SERVICE_CITIES: { id: string; name: string; lon: number; lat: number }[] = [
  { id: "MIA", name: "Miami", lon: -80.19, lat: 25.76 },
  { id: "FLL", name: "Fort Lauderdale", lon: -80.14, lat: 26.12 },
  { id: "PBI", name: "West Palm Beach", lon: -80.05, lat: 26.72 },
  { id: "EYW", name: "Cayo Hueso", lon: -81.78, lat: 24.56 },
  { id: "RSW", name: "Fort Myers", lon: -81.87, lat: 26.64 },
  { id: "TPA", name: "Tampa", lon: -82.46, lat: 27.95 },
  { id: "SRQ", name: "Sarasota", lon: -82.53, lat: 27.34 },
...
export async function fetchSpcOutlook(signal?: AbortSignal): Promise<SpcOutlook | null> {
  try {
    ...
    return {
      ...
      hail: arr("hail"),
      prob: arr("prob"),
      // population_by_level_FL (no la nacional): el titular de población debe
      // reflejar el ENCUADRE Florida. Sin fallback a la nacional a propósito.
      populationByLevel:
        d?.population_by_level_fl && typeof d.population_by_level_fl === "object"
          ? d.population_by_level_fl
          : undefined,
    };
  } catch (e) {
...
// FL. MIA ya viene en el feed nacional de tmax; el resto se publica desde nimbus
// (nbm_tmax_pipeline.py) igual que se hizo con las ciudades del NE/SE.
...
export async function fetchTmaxCities(
  ...
  try {
    ...
    if (r.ok) {
      ...
      // population_fl (no population): la tarjeta debe mostrar la población del
      // ENCUADRE Florida, no la nacional. Sin fallback a la nacional a propósito.
      popToday = pop(d?.population_fl?.today);
      popTomorrow = pop(d?.population_fl?.tomorrow);
    }
  } catch (e) {
...
// bbox REGIONAL para el filtro de terremotos: solo abre el vídeo de FLORIDA
// un sismo dentro del encuadre (mismo bbox que CONUS_VIEW; en FL la sismicidad
// es prácticamente nula, así que este bloque casi nunca saldrá). El nombre se
...
const CONUS_QUAKE_BBOX = { west: -87.9, east: -79.7, south: 24.3, north: 31.1 };
...
export const ESCALETA_SLUG = "tiempo-fl";
