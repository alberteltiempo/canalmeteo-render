...
// asoma inevitablemente (el bbox es un rectángulo) pero SIN ciudades ni datos de
// FL — Florida tendrá su propio segmento (tiempo_FL). Ajustable: ampliar oeste si
// se quiere Luisiana. (Mantiene el nombre CONUS_VIEW para no tocar las escenas.)
...
const CONDITION_CITIES: { key: string; name: string; lon: number; lat: number }[] = [
  ...
  { key: "Richmond|Virginia", name: "Richmond", lon: -77.46, lat: 37.54 },
  { key: "Raleigh|North Carolina", name: "Raleigh", lon: -78.64, lat: 35.78 },
  { key: "Columbia|South Carolina", name: "Columbia", lon: -81.03, lat: 34.0 },
...
// Aeropuertos del SE. Los que falten en el feed FAA de nimbus se añaden a
...
const AIRPORT_CATALOG: { iata: string; city: string; lon: number; lat: number }[] = [
  ...
  { iata: "BNA", city: "Nashville", lon: -86.68, lat: 36.12 },
  { iata: "MEM", city: "Memphis", lon: -89.98, lat: 35.04 },
  { iata: "RDU", city: "Raleigh", lon: -78.79, lat: 35.88 },
  { iata: "RIC", city: "Richmond", lon: -77.32, lat: 37.51 },
  { iata: "ORF", city: "Norfolk", lon: -76.2, lat: 36.89 },
  { iata: "SAV", city: "Savannah", lon: -81.2, lat: 32.13 },
  { iata: "CHS", city: "Charleston", lon: -80.04, lat: 32.9 },
  { iata: "BHM", city: "Birmingham", lon: -86.75, lat: 33.56 },
  { iata: "SDF", city: "Louisville", lon: -85.74, lat: 38.17 },
  { iata: "TYS", city: "Knoxville", lon: -83.99, lat: 35.81 },
  { iata: "JAN", city: "Jackson", lon: -90.08, lat: 32.31 },
  { iata: "MOB", city: "Mobile", lon: -88.24, lat: 30.69 },
];
...
// SE. Los ids que el feed de nimbus aún no publique se descartan hasta que
// uv_pipeline/aqi_pipeline los incluyan (mismo patrón que NE).
const SERVICE_CITIES: { id: string; name: string; lon: number; lat: number }[] = [
  { id: "ATL", name: "Atlanta", lon: -84.39, lat: 33.75 },
  { id: "CLT", name: "Charlotte", lon: -80.84, lat: 35.23 },
  { id: "RDU", name: "Raleigh", lon: -78.64, lat: 35.78 },
  { id: "BNA", name: "Nashville", lon: -86.78, lat: 36.16 },
  { id: "MEM", name: "Memphis", lon: -90.05, lat: 35.15 },
  { id: "BHM", name: "Birmingham", lon: -86.8, lat: 33.52 },
  { id: "RIC", name: "Richmond", lon: -77.46, lat: 37.54 },
  { id: "ORF", name: "Norfolk", lon: -76.29, lat: 36.85 },
  { id: "CAE", name: "Columbia", lon: -81.03, lat: 34.0 },
  { id: "SAV", name: "Savannah", lon: -81.1, lat: 32.08 },
  { id: "CHS", name: "Charleston", lon: -79.93, lat: 32.78 },
  { id: "JAN", name: "Jackson", lon: -90.18, lat: 32.3 },
  { id: "MOB", name: "Mobile", lon: -88.04, lat: 30.69 },
  { id: "SDF", name: "Louisville", lon: -85.76, lat: 38.25 },
];
...
export async function fetchSpcOutlook(signal?: AbortSignal): Promise<SpcOutlook | null> {
  try {
    ...
    return {
      ...
      hail: arr("hail"),
      prob: arr("prob"),
      // population_by_level_SE (no la nacional): el titular de población debe
      // reflejar el ENCUADRE Sureste. Sin fallback a la nacional a propósito.
      populationByLevel:
        d?.population_by_level_se && typeof d.population_by_level_se === "object"
          ? d.population_by_level_se
          : undefined,
    };
  } catch (e) {
...
// SE. ATL/MEM/BNA/RDU ya vienen en el feed nacional de tmax; el resto se publica
// desde nimbus (nbm_tmax_pipeline.py) igual que se hizo con las ciudades del NE.
const TMAX_CITY_CATALOG: { id: string; name: string; lon: number; lat: number }[] = [
  ...
  { id: "CAE", name: "Columbia", lon: -81.03, lat: 34.0 },
  { id: "SAV", name: "Savannah", lon: -81.1, lat: 32.08 },
  { id: "TYS", name: "Knoxville", lon: -83.92, lat: 35.96 },
  { id: "CHS", name: "Charleston", lon: -79.93, lat: 32.78 },
  { id: "JAN", name: "Jackson", lon: -90.18, lat: 32.3 },
  { id: "MOB", name: "Mobile", lon: -88.04, lat: 30.69 },
  { id: "SDF", name: "Louisville", lon: -85.76, lat: 38.25 },
...
const SAMPLE_TMAX: Record<string, number> = {
  ATL: 92, CLT: 93, RDU: 94, BNA: 93, MEM: 96, BHM: 94, RIC: 92, ORF: 89,
  CAE: 96, SAV: 94, TYS: 91, CHS: 92, JAN: 95, MOB: 91, SDF: 92,
};
...
export async function fetchTmaxCities(
  ...
  try {
    ...
    if (r.ok) {
      ...
      // population_se (no population): la tarjeta debe mostrar la población del
      // ENCUADRE Sureste, no la nacional. Sin fallback a la nacional a propósito.
      popToday = pop(d?.population_se?.today);
      popTomorrow = pop(d?.population_se?.tomorrow);
    }
  } catch (e) {
...
// bbox REGIONAL para el filtro de terremotos: solo abre el vídeo del SURESTE
// un sismo dentro del encuadre (mismo bbox que CONUS_VIEW; incluye la zona
// sísmica del este de Tennessee y parte de New Madrid). El nombre se conserva
// del clon. El feed de nimbus será nacional → el bbox se aplica también en la
// rama CDN de fetchQuake, no solo en el fallback USGS.
const CONUS_QUAKE_BBOX = { west: -92.5, east: -75.2, south: 29.8, north: 39.6 };
