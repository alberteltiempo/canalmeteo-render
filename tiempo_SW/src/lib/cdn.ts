...
// Encuadre del SUROESTE (CA·NV·AZ·UT), bbox [[oeste,sur],[este,norte]]. De
// la costa del Pacífico a la línea este de UT/AZ y de la frontera mexicana a
// la línea norte de NV/UT; los bordes de estados vecinos (OR·ID·WY·CO·NM)
// asoman pero SIN ciudades ni datos suyos. (Mantiene el nombre CONUS_VIEW.)
...
export const REGION_STATES = new Set(["CA", "NV", "AZ", "UT"]);
...
// Ciudades de Suroeste en weather.json (capitales + latinas del feed
...
const CONDITION_CITIES: { key: string; name: string; lon: number; lat: number }[] = [
  ...
  { key: "San Diego|California", name: "San Diego", lon: -117.16, lat: 32.72 },
  { key: "San Francisco|California", name: "San Francisco", lon: -122.42, lat: 37.77 },
  { key: "Sacramento|California", name: "Sacramento", lon: -121.49, lat: 38.58 },
  { key: "Fresno|California", name: "Fresno", lon: -119.77, lat: 36.75 },
  { key: "Las Vegas|Nevada", name: "Las Vegas", lon: -115.14, lat: 36.17 },
  { key: "Reno|Nevada", name: "Reno", lon: -119.81, lat: 39.53 },
  { key: "Phoenix|Arizona", name: "Phoenix", lon: -112.07, lat: 33.45 },
  { key: "Tucson|Arizona", name: "Tucson", lon: -110.97, lat: 32.22 },
...
// Aeropuertos de Suroeste. Los que falten en el feed FAA de nimbus se
...
const AIRPORT_CATALOG: { iata: string; city: string; lon: number; lat: number }[] = [
  ...
  { iata: "SJC", city: "San José", lon: -121.93, lat: 37.36 },
  { iata: "OAK", city: "Oakland", lon: -122.22, lat: 37.72 },
  { iata: "SMF", city: "Sacramento", lon: -121.59, lat: 38.7 },
  { iata: "BUR", city: "Burbank", lon: -118.36, lat: 34.2 },
  { iata: "LAS", city: "Las Vegas", lon: -115.15, lat: 36.08 },
  { iata: "RNO", city: "Reno", lon: -119.77, lat: 39.5 },
  { iata: "PHX", city: "Phoenix", lon: -112.01, lat: 33.43 },
  { iata: "TUS", city: "Tucson", lon: -110.94, lat: 32.12 },
...
// Suroeste. Los ids que el feed de nimbus aún no publique se descartan
...
const SERVICE_CITIES: { id: string; name: string; lon: number; lat: number }[] = [
  ...
  { id: "SAN", name: "San Diego", lon: -117.16, lat: 32.72 },
  { id: "SFO", name: "San Francisco", lon: -122.42, lat: 37.77 },
  { id: "SAC", name: "Sacramento", lon: -121.49, lat: 38.58 },
  { id: "FAT", name: "Fresno", lon: -119.77, lat: 36.75 },
  { id: "BFL", name: "Bakersfield", lon: -119.02, lat: 35.37 },
  { id: "PSP", name: "Palm Springs", lon: -116.55, lat: 33.83 },
  { id: "LAS", name: "Las Vegas", lon: -115.14, lat: 36.17 },
  { id: "RNO", name: "Reno", lon: -119.81, lat: 39.53 },
...
export async function fetchSpcOutlook(signal?: AbortSignal): Promise<SpcOutlook | null> {
  try {
    ...
    return {
      ...
      hail: arr("hail"),
      prob: arr("prob"),
      // population_by_level_SW (no la nacional): el titular de población debe
      // reflejar el ENCUADRE Suroeste. Sin fallback a propósito.
      populationByLevel:
        d?.population_by_level_sw && typeof d.population_by_level_sw === "object"
          ? d.population_by_level_sw
          : undefined,
    };
  } catch (e) {
...
// Suroeste. Los ids que ya publica el feed nacional de tmax se usan tal
...
export async function fetchTmaxCities(
  ...
  try {
    ...
    if (r.ok) {
      ...
      // population_sw (no population): la tarjeta debe mostrar la población del
      // ENCUADRE Suroeste, no la nacional. Sin fallback a propósito.
      popToday = pop(d?.population_sw?.today);
      popTomorrow = pop(d?.population_sw?.tomorrow);
    }
  } catch (e) {
...
export async function fetchStormReports(signal?: AbortSignal): Promise<StormReportsData | null> {
  try {
    ...
    if (!r.ok) return null;
    const d = await r.json();
    // Bloque REGIONAL del feed (regions.sw): total/summary/puntos SOLO del
...
// del SUROESTE un sismo dentro del encuadre (San Andrés y compañía: aquí el
// bloque de terremoto es el más probable de toda la familia).
// El feed de nimbus será nacional → el bbox se aplica también en la rama CDN
// de fetchQuake, no solo en el fallback USGS.
const CONUS_QUAKE_BBOX = { west: -124.6, east: -108.9, south: 31.2, north: 42.2 };
...
// ─────────────────────────────────────────────────────────────
export const ESCALETA_SLUG = "tiempo-sw";
