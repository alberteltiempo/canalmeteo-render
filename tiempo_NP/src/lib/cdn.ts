...
// Encuadre de las LLANURAS DEL NORTE (MT·WY·CO·ND·SD·NE·KS), bbox [[oeste,
// sur],[este,norte]]. Del oeste de Montana a la frontera con Misuri/Iowa y
// del sur de Kansas/Colorado a la frontera canadiense; los bordes de estados
// vecinos asoman pero SIN ciudades ni datos suyos. (Nombre CONUS_VIEW.)
...
export const REGION_STATES = new Set(["MT", "WY", "CO", "ND", "SD", "NE", "KS"]);
...
// Ciudades de Llanuras del Norte en weather.json (capitales + latinas del feed
// nacional; el resto las inyecta cities_weather_pipeline.py en nimbus vía
// observaciones NWS — EXTRA_CITIES). Si el feed no trae alguna, no se pinta.
const CONDITION_CITIES: { key: string; name: string; lon: number; lat: number }[] = [
  { key: "Denver|Colorado", name: "Denver", lon: -104.99, lat: 39.74 },
  { key: "Cheyenne|Wyoming", name: "Cheyenne", lon: -104.82, lat: 41.14 },
  { key: "Helena|Montana", name: "Helena", lon: -112.04, lat: 46.59 },
  { key: "Bismarck|North Dakota", name: "Bismarck", lon: -100.78, lat: 46.81 },
  { key: "Billings|Montana", name: "Billings", lon: -108.5, lat: 45.78 },
  { key: "Fargo|North Dakota", name: "Fargo", lon: -96.79, lat: 46.88 },
  { key: "Rapid City|South Dakota", name: "Rapid City", lon: -103.23, lat: 44.08 },
  { key: "Sioux Falls|South Dakota", name: "Sioux Falls", lon: -96.73, lat: 43.55 },
  { key: "Omaha|Nebraska", name: "Omaha", lon: -95.93, lat: 41.26 },
...
// Aeropuertos de Llanuras del Norte. Los que falten en el feed FAA de nimbus se
// añaden a DELAYS_IATA allí; hasta entonces se descartan (el catálogo decide).
const AIRPORT_CATALOG: { iata: string; city: string; lon: number; lat: number }[] = [
  { iata: "DEN", city: "Denver", lon: -104.67, lat: 39.86 },
  { iata: "COS", city: "Colorado Springs", lon: -104.7, lat: 38.81 },
  { iata: "BIL", city: "Billings", lon: -108.54, lat: 45.81 },
  { iata: "MSO", city: "Missoula", lon: -114.09, lat: 46.92 },
  { iata: "BIS", city: "Bismarck", lon: -100.75, lat: 46.77 },
  { iata: "FAR", city: "Fargo", lon: -96.82, lat: 46.92 },
  { iata: "RAP", city: "Rapid City", lon: -103.06, lat: 44.05 },
  { iata: "FSD", city: "Sioux Falls", lon: -96.74, lat: 43.58 },
  { iata: "OMA", city: "Omaha", lon: -95.89, lat: 41.3 },
  { iata: "LNK", city: "Lincoln", lon: -96.75, lat: 40.85 },
...
// Llanuras del Norte. Los ids que el feed de nimbus aún no publique se descartan
// hasta que uv_pipeline/aqi_pipeline los incluyan (mismo patrón que el resto).
const SERVICE_CITIES: { id: string; name: string; lon: number; lat: number }[] = [
  { id: "DEN", name: "Denver", lon: -104.99, lat: 39.74 },
  { id: "COS", name: "Colorado Springs", lon: -104.82, lat: 38.83 },
  { id: "BIL", name: "Billings", lon: -108.5, lat: 45.78 },
  { id: "MSO", name: "Missoula", lon: -113.99, lat: 46.87 },
  { id: "GTF", name: "Great Falls", lon: -111.3, lat: 47.51 },
  { id: "CYS", name: "Cheyenne", lon: -104.82, lat: 41.14 },
  { id: "CPR", name: "Casper", lon: -106.31, lat: 42.87 },
  { id: "BIS", name: "Bismarck", lon: -100.78, lat: 46.81 },
  { id: "FAR", name: "Fargo", lon: -96.79, lat: 46.88 },
  { id: "RAP", name: "Rapid City", lon: -103.23, lat: 44.08 },
  { id: "FSD", name: "Sioux Falls", lon: -96.73, lat: 43.55 },
  { id: "OMA", name: "Omaha", lon: -95.93, lat: 41.26 },
  { id: "LNK", name: "Lincoln", lon: -96.7, lat: 40.81 },
...
export async function fetchSpcOutlook(signal?: AbortSignal): Promise<SpcOutlook | null> {
  try {
    ...
    return {
      ...
      hail: arr("hail"),
      prob: arr("prob"),
      // population_by_level_NP (no la nacional): el titular de población debe
      // reflejar el ENCUADRE Llanuras del Norte. Sin fallback a propósito.
      populationByLevel:
        d?.population_by_level_np && typeof d.population_by_level_np === "object"
          ? d.population_by_level_np
          : undefined,
    };
  } catch (e) {
...
// Llanuras del Norte. Los ids que ya publica el feed nacional de tmax se usan tal
// cual; el resto los publica nimbus (nbm_tmax_pipeline.py) como en el resto.
...
// valores de MUESTRA las ciudades del catálogo que el feed no trae y simula la
// máxima de HOY en runs de tarde. PONER A false en producción (entonces solo se
...
const SAMPLE_TMAX: Record<string, number> = {
  DEN: 95, COS: 90, BIL: 92, MSO: 93, GTF: 88, CYS: 87, CPR: 91, BIS: 89, FAR: 86, RAP: 92, FSD: 88, OMA: 92, LNK: 94, ICT: 98, TOP: 96,
};
...
export async function fetchTmaxCities(
  ...
  try {
    ...
    if (r.ok) {
      ...
      // population_np (no population): la tarjeta debe mostrar la población del
      // ENCUADRE Llanuras del Norte, no la nacional. Sin fallback a propósito.
      popToday = pop(d?.population_np?.today);
      popTomorrow = pop(d?.population_np?.tomorrow);
    }
  } catch (e) {
...
export async function fetchStormReports(signal?: AbortSignal): Promise<StormReportsData | null> {
  try {
    ...
    if (!r.ok) return null;
    const d = await r.json();
    // Bloque REGIONAL del feed (regions.np): total/summary/puntos SOLO del
...
// el MISMO filtro de producción: M≥5.5, dentro del bbox regional (SE) y últimas 6 h.
// Así solo abre el vídeo un sismo realmente cualificado (no cualquier microsismo).
export const QUAKE_TEST = true;
const USGS_DAY_URL =
  "https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/4.5_day.geojson";
// bbox REGIONAL para el filtro de terremotos: solo abre el vídeo
// de las LLANURAS DEL NORTE un sismo dentro del encuadre (incluye la zona
// de Yellowstone/oeste de Montana, con sismicidad real).
// El feed de nimbus será nacional → el bbox se aplica también en la rama CDN
// de fetchQuake, no solo en el fallback USGS.
const CONUS_QUAKE_BBOX = { west: -116.2, east: -94.5, south: 36.9, north: 49.1 };
...
export async function fetchQuake(signal?: AbortSignal): Promise<Quake | null> {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // dentro del encuadre Llanuras del Norte.
      ...
      if (q && freshEnough && !inRegion && typeof q.mag === "number") {
        console.warn("[conus] quake (cdn): fuera del encuadre SE, no abre el vídeo");
      }
      if (
        q &&
        freshEnough &&
  ...
  // regional SE, últimas 6 h). El más fuerte de los que cualifican.
  if (QUAKE_TEST) {
    try {
...
// en el CDN con escenas on/off y duraciones. La web lo publica; aquí se
...
// ─────────────────────────────────────────────────────────────
export const ESCALETA_SLUG = "tiempo-np";
