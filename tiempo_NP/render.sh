#!/bin/bash
# render.sh — Canal Meteo · Vistazo al Tiempo EEUU (LLANURAS DEL NORTE)
# ─────────────────────────────────────────────────────────────────────
# Renderiza el segmento LLANURAS DEL NORTE (auto-generado desde el CDN —satélite GOES,
# radar MRMS y vigilancias NWS— vía calculateMetadata), valida el MP4, lo
# sube al CDN y a Dropbox (/Playout/RenderFarm/TIEMPO_NP.mp4), y escribe
...
#   ./render.sh --gl=swangle             # fuerza renderer software (máquina sin GPU)
#   ./render.sh --triggered-by=cron      # etiqueta de origen para el status
# ─────────────────────────────────────────────────────────────────────
...
PROJECT_DIR="/opt/canalmeteo/remotion/tiempo_NP"
...
NO_UPLOAD=false
TRIGGERED_BY="manual"
# 4 (no 8): las escenas ahora drapean raster pesado (relieve + IR/radar) en Mapbox;
# con 8 pestañas WebGL a la vez la GPU se quedaba sin memoria ("Failed to initialize WebGL").
CONCURRENCY=4
GL="vulkan"   # calamarsa tiene GPU; usa --gl=swangle en máquinas sin GPU
...
# ─── Identidad del producto (horizontal broadcast/web 1920×1080) ──────
COMPOSITION="NpSegment"
SLUG="tiempo-np"
DIRNAME="tiempo-np"
DROPBOX_NAME="TIEMPO_NP.mp4"
LABEL="Vistazo al Tiempo EEUU · LLANURAS DEL NORTE (horizontal 1920×1080)"

OUTPUT_DIR="/opt/canalmeteo/data/videos/$DIRNAME"
mkdir -p "$OUTPUT_DIR" "$TEMP_DIR" "$LOG_DIR" "$STATUS_DIR"
cd "$PROJECT_DIR"

# ─── Lock GLOBAL de GPU (compartido con Trópico, CONUS y NE) ──────
...
# ocupada, desiste (el próximo cron reintenta). El flock del crontab
# (/tmp/np-render.lock) sigue evitando que se apilen NP con NP.
...
EST_TOTAL_FRAMES=945   # ~31.5 s @ 30 fps (real lo detecta Remotion)
...
log "  Concurrency:  $CONCURRENCY  ·  GL: $GL"
...
# otro render en marcha (Trópico/CONUS/NE) antes de existir el lock global.
