...
// Ciudades del NE presentes en weather.json (capitales estatales + NYC/Boston).
// Washington, Filadelfia, Baltimore y Pittsburgh NO están en el feed (es por
// capitales): usamos Annapolis/Harrisburg como referencia de su zona.
const CONDITION_CITIES: { key: string; name: string; lon: number; lat: number }[] = [
  ...
  { key: "Boston|Massachusetts", name: "Boston", lon: -71.06, lat: 42.36 },
  { key: "Hartford|Connecticut", name: "Hartford", lon: -72.69, lat: 41.76 },
  { key: "Providence|Rhode Island", name: "Providence", lon: -71.41, lat: 41.82 },
  { key: "Albany|New York", name: "Albany", lon: -73.76, lat: 42.65 },
  { key: "Harrisburg|Pennsylvania", name: "Harrisburg", lon: -76.88, lat: 40.27 },
  { key: "Trenton|New Jersey", name: "Trenton", lon: -74.76, lat: 40.22 },
  { key: "Concord|New Hampshire", name: "Concord", lon: -71.54, lat: 43.21 },
  { key: "Montpelier|Vermont", name: "Montpelier", lon: -72.58, lat: 44.26 },
...
// Aeropuertos del NE. En el feed FAA actual hay: JFK, EWR, LGA, BOS, PHL, BWI.
// El resto se incluye para cuando el feed los publique (se descartan si no vienen).
const AIRPORT_CATALOG: { iata: string; city: string; lon: number; lat: number }[] = [
  ...
  { iata: "DCA", city: "Washington", lon: -77.04, lat: 38.85 },
  { iata: "IAD", city: "Washington-Dulles", lon: -77.46, lat: 38.95 },
  { iata: "PIT", city: "Pittsburgh", lon: -80.25, lat: 40.49 },
  { iata: "BUF", city: "Buffalo", lon: -78.73, lat: 42.94 },
  { iata: "BDL", city: "Hartford", lon: -72.68, lat: 41.94 },
  { iata: "PVD", city: "Providence", lon: -71.42, lat: 41.72 },
  { iata: "ALB", city: "Albany", lon: -73.8, lat: 42.75 },
...
// NE. OJO: el feed actual de UV trae solo NYC; el de AQI, NYC y BOS. El resto se
// dibujará cuando nimbus publique esos ids (hasta entonces se descartan).
const SERVICE_CITIES: { id: string; name: string; lon: number; lat: number }[] = [
  ...
  { id: "PHL", name: "Filadelfia", lon: -75.16, lat: 39.95 },
  { id: "DC", name: "Washington", lon: -77.04, lat: 38.91 },
  { id: "BAL", name: "Baltimore", lon: -76.61, lat: 39.29 },
  { id: "PIT", name: "Pittsburgh", lon: -79.996, lat: 40.44 },
  { id: "BUF", name: "Buffalo", lon: -78.87, lat: 42.89 },
  { id: "ALB", name: "Albany", lon: -73.76, lat: 42.65 },
...
// NE. El feed de tmax trae hoy NYC y BOS; el resto se dibuja con TMAX_TEST (abajo)
// o cuando nimbus publique esos ids en data/tmax/cities.json.
...
export async function fetchStormReports(signal?: AbortSignal): Promise<StormReportsData | null> {
  try {
    ...
    if (!r.ok) return null;
    const d = await r.json();
    // Bloque REGIONAL del feed (regions.ne): total/summary/puntos SOLO del
...
export const ESCALETA_SLUG = "tiempo-ne";
