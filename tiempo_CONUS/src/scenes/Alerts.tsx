...
export const Alerts: React.FC<{
  ...
  const conocidas = (alerts?.watches || []).filter((a) => !!a.tipo && a.tipo in ALERT_CATEGORIES);
  const grandes = conocidas.filter((a) => esZonaGrande(a.tipo, a.is_grouped));
  // Sin vigilancias de zona grande pero con avisos pintables → las cajas resumen
  // esos avisos, para no decir "sin alertas" con el mapa pintado.
  const allWatches = grandes.length ? grandes : conocidas;
  const groups = groupWatches(allWatches, categories);
  const hasWatches = groups.length > 0;

  // Polígonos: solo categorías conocidas (las mismas que pueden salir en cajas) —
  // lo pintado y lo contado es lo mismo.
