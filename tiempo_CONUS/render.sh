...
PROJECT_DIR="/opt/canalmeteo/remotion/tiempo_CONUS"
...
COMPOSITION="ConusSegment"
SLUG="tiempo-conus"
DIRNAME="tiempo-conus"
DROPBOX_NAME="TIEMPO_CONUS.mp4"
LABEL="Vistazo al Tiempo EEUU · CONUS (horizontal 1920×1080)"

OUTPUT_DIR="/opt/canalmeteo/data/videos/$DIRNAME"
mkdir -p "$OUTPUT_DIR" "$TEMP_DIR" "$LOG_DIR" "$STATUS_DIR"
cd "$PROJECT_DIR"

# ─── Lock GLOBAL de GPU (compartido con Trópico y tiempo_NE) ─────────
...
# (/tmp/conus-render.lock) sigue evitando que se apilen CONUS con CONUS.
...
# otro render en marcha (Trópico/NE) antes de existir el lock global.
