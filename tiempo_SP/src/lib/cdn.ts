...
// Encuadre de las LLANURAS DEL SUR (TX·OK·AR·LA·NM), bbox [[oeste,sur],[este,
// norte]]. De la frontera oeste de NM al delta del Misisipi y del valle del Río
// Grande (Brownsville) a la línea norte de OK/NM; los bordes de estados vecinos
// (KS·MO·MS·CO·AZ) asoman inevitablemente pero SIN ciudades ni datos suyos.
// (Mantiene el nombre CONUS_VIEW para no tocar las escenas.)
...
export const REGION_STATES = new Set(["TX", "OK", "AR", "LA", "NM"]);
...
// Ciudades de las Llanuras del Sur presentes en weather.json (capitales +
// latinas del feed nacional). El Paso y Albuquerque NO están en el feed: las
// inyecta cities_weather_pipeline.py en nimbus con observaciones NWS (mismo
// mecanismo que Nueva Orleans) — si el feed aún no las trae, no se pintan.
const CONDITION_CITIES: { key: string; name: string; lon: number; lat: number }[] = [
  ...
  { key: "San Antonio|Texas", name: "San Antonio", lon: -98.49, lat: 29.42 },
  { key: "Austin|Texas", name: "Austin", lon: -97.74, lat: 30.27 },
  { key: "El Paso|Texas", name: "El Paso", lon: -106.49, lat: 31.76 },
...
// Aeropuertos de las Llanuras del Sur. Los que falten en el feed FAA de nimbus
// se añaden a DELAYS_IATA allí; hasta entonces se descartan (el catálogo decide
// qué se dibuja).
const AIRPORT_CATALOG: { iata: string; city: string; lon: number; lat: number }[] = [
  { iata: "DFW", city: "Dallas-Fort Worth", lon: -97.04, lat: 32.9 },
  { iata: "IAH", city: "Houston", lon: -95.34, lat: 29.98 },
  { iata: "AUS", city: "Austin", lon: -97.67, lat: 30.19 },
  { iata: "SAT", city: "San Antonio", lon: -98.47, lat: 29.53 },
  { iata: "ELP", city: "El Paso", lon: -106.38, lat: 31.81 },
  { iata: "OKC", city: "Oklahoma City", lon: -97.6, lat: 35.39 },
  { iata: "TUL", city: "Tulsa", lon: -95.89, lat: 36.2 },
  { iata: "LIT", city: "Little Rock", lon: -92.22, lat: 34.73 },
  { iata: "MSY", city: "Nueva Orleans", lon: -90.26, lat: 29.99 },
  { iata: "ABQ", city: "Albuquerque", lon: -106.61, lat: 35.04 },
  { iata: "LBB", city: "Lubbock", lon: -101.82, lat: 33.66 },
  { iata: "AMA", city: "Amarillo", lon: -101.71, lat: 35.22 },
];
...
// Llanuras del Sur. Los ids que el feed de nimbus aún no publique se descartan
// hasta que uv_pipeline/aqi_pipeline los incluyan (mismo patrón que NE/SE/FL).
const SERVICE_CITIES: { id: string; name: string; lon: number; lat: number }[] = [
  ...
  { id: "SAT", name: "San Antonio", lon: -98.49, lat: 29.42 },
  { id: "AUS", name: "Austin", lon: -97.74, lat: 30.27 },
  { id: "ELP", name: "El Paso", lon: -106.49, lat: 31.76 },
  { id: "CRP", name: "Corpus Christi", lon: -97.4, lat: 27.8 },
  { id: "LBB", name: "Lubbock", lon: -101.86, lat: 33.58 },
  { id: "AMA", name: "Amarillo", lon: -101.83, lat: 35.19 },
  { id: "OKC", name: "Oklahoma City", lon: -97.52, lat: 35.47 },
  { id: "TUL", name: "Tulsa", lon: -95.99, lat: 36.15 },
...
export async function fetchSpcOutlook(signal?: AbortSignal): Promise<SpcOutlook | null> {
  try {
    ...
    return {
      ...
      hail: arr("hail"),
      prob: arr("prob"),
      // population_by_level_SP (no la nacional): el titular de población debe
      // reflejar el ENCUADRE Llanuras del Sur. Sin fallback a propósito.
      populationByLevel:
        d?.population_by_level_sp && typeof d.population_by_level_sp === "object"
          ? d.population_by_level_sp
          : undefined,
    };
  } catch (e) {
...
// Llanuras del Sur. DAL/HOU/OKC/ABQ/MSY ya vienen en el feed nacional de tmax;
// el resto se publica desde nimbus (nbm_tmax_pipeline.py) como en NE/SE/FL.
...
export async function fetchTmaxCities(
  ...
  try {
    ...
    if (r.ok) {
      ...
      // population_sp (no population): la tarjeta debe mostrar la población del
      // ENCUADRE Llanuras del Sur, no la nacional. Sin fallback a propósito.
      popToday = pop(d?.population_sp?.today);
      popTomorrow = pop(d?.population_sp?.tomorrow);
    }
  } catch (e) {
...
export async function fetchStormReports(signal?: AbortSignal): Promise<StormReportsData | null> {
  try {
    ...
    if (!r.ok) return null;
    const d = await r.json();
    // Bloque REGIONAL del feed (regions.sp): total/summary/puntos SOLO del
...
// bbox REGIONAL para el filtro de terremotos: solo abre el vídeo de las
// LLANURAS DEL SUR un sismo dentro del encuadre (mismo bbox que CONUS_VIEW;
// incluye el oeste de TX/Permian y el borde de New Madrid en AR). El nombre se
// conserva del clon. El feed de nimbus será nacional → el bbox se aplica también
// en la rama CDN de fetchQuake, no solo en el fallback USGS.
const CONUS_QUAKE_BBOX = { west: -109.1, east: -89.3, south: 25.7, north: 37.2 };
...
export async function fetchQuake(signal?: AbortSignal): Promise<Quake | null> {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // dentro del encuadre Llanuras del Sur.
...
export const ESCALETA_SLUG = "tiempo-sp";
