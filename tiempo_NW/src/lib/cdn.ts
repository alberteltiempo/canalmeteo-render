...
// Encuadre del NOROESTE (WA·OR·ID), bbox [[oeste,sur],[este,norte]]. De la
// costa del Pacífico al este de Idaho y de la línea sur de Oregón/Idaho a la
// frontera canadiense; los bordes de estados vecinos (CA·NV·UT·MT·WY) asoman
...
export const REGION_STATES = new Set(["WA", "OR", "ID"]);
...
// Ciudades de Noroeste en weather.json (capitales + latinas del feed
...
const CONDITION_CITIES: { key: string; name: string; lon: number; lat: number }[] = [
  { key: "Seattle|Washington", name: "Seattle", lon: -122.33, lat: 47.61 },
  { key: "Portland|Oregon", name: "Portland", lon: -122.68, lat: 45.52 },
  { key: "Boise|Idaho", name: "Boise", lon: -116.2, lat: 43.62 },
  { key: "Spokane|Washington", name: "Spokane", lon: -117.43, lat: 47.66 },
  { key: "Olympia|Washington", name: "Olympia", lon: -122.9, lat: 47.04 },
  { key: "Salem|Oregon", name: "Salem", lon: -123.04, lat: 44.94 },
  { key: "Eugene|Oregon", name: "Eugene", lon: -123.09, lat: 44.05 },
  { key: "Idaho Falls|Idaho", name: "Idaho Falls", lon: -112.03, lat: 43.49 },
];
...
// Aeropuertos de Noroeste. Los que falten en el feed FAA de nimbus se
...
const AIRPORT_CATALOG: { iata: string; city: string; lon: number; lat: number }[] = [
  ...
  { iata: "BOI", city: "Boise", lon: -116.22, lat: 43.56 },
  { iata: "GEG", city: "Spokane", lon: -117.53, lat: 47.62 },
  { iata: "EUG", city: "Eugene", lon: -123.21, lat: 44.12 },
  { iata: "MFR", city: "Medford", lon: -122.87, lat: 42.37 },
  { iata: "PSC", city: "Tri-Cities", lon: -119.12, lat: 46.26 },
  { iata: "IDA", city: "Idaho Falls", lon: -112.07, lat: 43.51 },
];
...
// Noroeste. Los ids que el feed de nimbus aún no publique se descartan
...
const SERVICE_CITIES: { id: string; name: string; lon: number; lat: number }[] = [
  ...
  { id: "GEG", name: "Spokane", lon: -117.43, lat: 47.66 },
  { id: "YKM", name: "Yakima", lon: -120.51, lat: 46.6 },
  { id: "EUG", name: "Eugene", lon: -123.09, lat: 44.05 },
  { id: "MFR", name: "Medford", lon: -122.87, lat: 42.33 },
  { id: "RDM", name: "Bend", lon: -121.31, lat: 44.06 },
  { id: "SLE", name: "Salem", lon: -123.04, lat: 44.94 },
  { id: "IDA", name: "Idaho Falls", lon: -112.03, lat: 43.49 },
  { id: "PIH", name: "Pocatello", lon: -112.44, lat: 42.86 },
  { id: "LWS", name: "Lewiston", lon: -117.02, lat: 46.42 },
  { id: "PSC", name: "Tri-Cities", lon: -119.12, lat: 46.23 },
  { id: "BLI", name: "Bellingham", lon: -122.48, lat: 48.75 },
];
...
export async function fetchSpcOutlook(signal?: AbortSignal): Promise<SpcOutlook | null> {
  try {
    ...
    return {
      ...
      hail: arr("hail"),
      prob: arr("prob"),
      // population_by_level_NW (no la nacional): el titular de población debe
      // reflejar el ENCUADRE Noroeste. Sin fallback a propósito.
      populationByLevel:
        d?.population_by_level_nw && typeof d.population_by_level_nw === "object"
          ? d.population_by_level_nw
          : undefined,
    };
  } catch (e) {
...
// Noroeste. Los ids que ya publica el feed nacional de tmax se usan tal
...
const SAMPLE_TMAX: Record<string, number> = {
  SEA: 78, PDX: 84, BOI: 96, GEG: 89, YKM: 92, EUG: 85, MFR: 95, RDM: 88, SLE: 86, IDA: 90, PIH: 91, LWS: 97, PSC: 95, BLI: 74,
};
...
export async function fetchTmaxCities(
  ...
  try {
    ...
    if (r.ok) {
      ...
      // population_nw (no population): la tarjeta debe mostrar la población del
      // ENCUADRE Noroeste, no la nacional. Sin fallback a propósito.
      popToday = pop(d?.population_nw?.today);
      popTomorrow = pop(d?.population_nw?.tomorrow);
    }
  } catch (e) {
...
export async function fetchStormReports(signal?: AbortSignal): Promise<StormReportsData | null> {
  try {
    ...
    if (!r.ok) return null;
    const d = await r.json();
    // Bloque REGIONAL del feed (regions.nw): total/summary/puntos SOLO del
...
// del NOROESTE un sismo dentro del encuadre (Cascadia y el oeste de WA/OR
// tienen sismicidad real y relevante).
// El feed de nimbus será nacional → el bbox se aplica también en la rama CDN
// de fetchQuake, no solo en el fallback USGS.
const CONUS_QUAKE_BBOX = { west: -124.9, east: -110.9, south: 41.8, north: 49.1 };
...
// ─────────────────────────────────────────────────────────────
export const ESCALETA_SLUG = "tiempo-nw";
